...
class RandTrajEnv(DroneEnv):
    ...
    def __init__(
        ...
        *,
        ...
        # NEW: obstacle/gate features
        spec["nearest_obstacle_rel"] = spaces.Box(-np.inf, np.inf, shape=(3,))
        spec["nearest_obstacle_dist"] = spaces.Box(-np.inf, np.inf, shape=(1,))
        spec["target_gate_rel"] = spaces.Box(-np.inf, np.inf, shape=(3,))
        spec["target_gate_local_yz"] = spaces.Box(-np.inf, np.inf, shape=(2,))
    ...
    def obs(self) -> dict[str, Array]:
        ...
        pos = self.sim.data.states.pos[:, 0, :]  # (num_envs, 3)
        ...
        obs["nearest_obstacle_rel"] = d_obs[batch_idx, nearest_idx]
        obs["nearest_obstacle_dist"] = dist_obs[batch_idx, nearest_idx][:, None]

        # --------------------------------------------------------
        # Target gate feature
        # --------------------------------------------------------
        target_gate = self.sim.data.target_gate[:, 0]  # (num_envs,)
        n_gates = self.sim.data.gates_pos.shape[1]
        safe_target_gate = jp.clip(target_gate, 0, n_gates - 1)

        gates_pos = self.sim.data.gates_pos
        gates_quat = self.sim.data.gates_quat

        gate_pos = gates_pos[batch_idx, safe_target_gate]
        gate_quat = gates_quat[batch_idx, safe_target_gate]

        obs["target_gate_rel"] = gate_pos - pos

        # Position of drone in local gate frame
        pos_local = R.from_quat(gate_quat).apply(pos - gate_pos, inverse=True)

        # y/z decides whether drone is close to gate edges
        obs["target_gate_local_yz"] = pos_local[:, 1:3]
    ...
    def reward(self) -> Array:
        ...
        # Base trajectory tracking reward
        ...
        # --------------------------------------------------------
        obstacle_dist = obs["nearest_obstacle_dist"].squeeze(-1)
        ...
        # --------------------------------------------------------
        # Gate edge penalty
        # Gate opening is checked elsewhere with gate_size=(0.45, 0.45),
        # therefore half gate size is 0.225.
        # --------------------------------------------------------
        gate_yz = obs["target_gate_local_yz"]
        ...
        # Strong crash/termination penalty
        reward = jp.where(self.terminated(), -10.0, reward)
