...
Action / observation / weight layout all live in :mod:`weight_policy` (shared with
deployment). Reward = path progress + gate-pass bonus − crashes − failed solves − time.

Run from the repo root::

    python lsy_drone_racing/control/mpcc/train_weights.py --train True --eval 2

Quick smoke test (few steps, 2 envs)::

    python lsy_drone_racing/control/mpcc/train_weights.py --total_timesteps 4000 --num_envs 2
...
from lsy_drone_racing.control.mpcc.controller import MPCCController
from lsy_drone_racing.control.mpcc.weight_policy import WeightPolicyNet
