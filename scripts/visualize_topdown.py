...
def draw_gate_topdown(ax, center, quat, label):
    ...
    x_axis = rot.apply([1., 0., 0.])[:2]  # approach direction (thin dimension)
    y_axis = rot.apply([0., 1., 0.])[:2]  # gate width direction (wide dimension)
    ...
    # Approach arrow
    ax.annotate("", xy=(cx + x_axis[0] * 0.5, cy + x_axis[1] * 0.5),
                xytext=(cx - x_axis[0] * 0.5, cy - x_axis[1] * 0.5),
                arrowprops=dict(arrowstyle="->", color="darkred", lw=1.5), zorder=4)

    # Label
    ax.text(cx + 0.08, cy + 0.08, label, color="red", fontsize=10,
...
def main():
    ...
    # Arena boundary
    boundary = plt.Polygon([[-2.5, -1.5], [2.5, -1.5], [2.5, 1.5], [-2.5, 1.5]],
                            fill=False, edgecolor="gray", lw=1.5, linestyle="--")
    ...
    wx = [p[0] for p in wp_pos]; wy = [p[1] for p in wp_pos]
    ...
    for i, (p, v) in enumerate(zip(wp_pos, wp_vel)):
        ...
        ax.text(p[0] + 0.04, p[1] + 0.04, str(i), color=color, fontsize=8, fontweight="bold", zorder=8)
    ...
    for i, opos in enumerate(obs["obstacles_pos"]):
        ...
        ax.add_patch(plt.Circle(opos[:2], radius=0.085, color="orange",
                                alpha=0.3, zorder=3))
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title("Left-click to sketch intended path  |  Backspace = undo  |  Enter = print points  |  'c' = clear")
